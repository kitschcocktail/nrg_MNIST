...
\begin{algorithm}[H]
 ...
 \KwResult{digit class}
  ...
  \eIf{$\mathcal{F} > threshold$}{
   ...
   mix corrupted image with generated image\;
   reclassify\;
   }{
   return class\;
  }
...
\subsection{Analysis}
While running the reclassification and recontruction trials, we saw that there was an important drop in error rates in the classification of the mixed images, but what's more is that reclassification alone was even more successful.
		...
		\subtable[Reconstruction]{
			\pgfplotstabletypeset[
				...
				every head row/.style={%
					...
					before row={%
						\cline{2-17}
						& \multicolumn{4}{|c|}{1 \% corruption}%
						& \multicolumn{4}{c|}{2 \% corruption}%
						& \multicolumn{4}{c|}{5 \% corruption}%
						& \multicolumn{4}{c|}{10 \% corruption}\\
					},
					after row=\hline
				},
				every last row/.style={after row=\hline},
